...
export default function Home() {
  ...
  useEffect(() => {
    ...
    setFilterState(prev => ({
      ...
      consultationType: (params.get('consultation') as 'Video Consult' | 'In Clinic' | null),
      specialties: params.get('specialties')?.split(',').filter(Boolean) || [],
      sortBy: (params.get('sortBy') as 'fees' | 'experience' | null),
      sortOrder: (params.get('sortOrder') as 'asc' | 'desc' | null),
    }));
  }, []);
  ...
  useEffect(() => {
    ...
    if (filterState.consultationType) params.set('consultation', filterState.consultationType);
    if (filterState.specialties.length > 0) params.set('specialties', filterState.specialties.join(','));
